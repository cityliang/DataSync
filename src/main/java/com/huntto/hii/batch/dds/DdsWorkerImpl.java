...
@Slf4j
@Component
public class DdsWorkerImpl extends DdsWorker {
	...
	public void execute() {
		...
		//入参
		String ddsDate = params.get(PARAM_DDS_DATE);
		String ddsName = params.get(PARAM_DDS_NAME);
...
//			stmt = SessionFactoryUtils.getDataSource(hibernateUtil.getSessionFactory()).getConnection().createStatement();
			tx = hibernateUtil.currentSession().beginTransaction();
			stmt = hibernateUtil.currentSession().doReturningWork(new ReturningWork<Statement>() {
				@Override
